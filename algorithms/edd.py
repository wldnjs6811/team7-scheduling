...
Objective        : Comparison based on Total Completion Time  ΣCj
                   (EDD is mainly a due-date-aware heuristic here)
...
def compute_metrics(results: list[dict]) -> dict:
    """
    Compute summary metrics for EDD results.
    """
    n = len(results)
...
def run_edd(jobs: list[dict]) -> tuple[list[dict], dict, float]:
    """
    Execute EDD using a list of job dictionaries.
...
def print_edd_results(results: list[dict], metrics: dict, runtime: float) -> None:
    """
    Print EDD scheduling results.
    """
    print(f"\n{'=' * 50}")
    print("  EDD Scheduling Results")
    print(f"{'=' * 50}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print_edd_results(results, metrics, runtime)

    # Case 2. No CSV file path: guide for Team 7 dataset usage
    else:
        print("EDD module is ready.")
        ...
        print("from edd_final import run_edd")
        print()
        print("jobs = get_small_jobs()")
        print("results, metrics, runtime = run_edd(jobs)")
