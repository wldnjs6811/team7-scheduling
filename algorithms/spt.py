...
Objective        : Minimize / compare Total Completion Time  ΣCj
...
and is no longer guaranteed optimal, but remains a strong comparison rule.
...
def compute_metrics(results: list[dict]) -> dict:
    """
    Compute summary metrics for SPT results.
    """
    n = len(results)
...
def run_spt(jobs: list[dict]) -> tuple[list[dict], dict, float]:
    """
    Execute SPT using a list of job dictionaries.
...
def print_spt_results(results: list[dict], metrics: dict, runtime: float) -> None:
    """
    Print SPT scheduling results.
    """
    print(f"\n{'=' * 50}")
    print("  SPT Scheduling Results")
    print(f"{'=' * 50}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print_spt_results(results, metrics, runtime)

    # Case 2. No CSV file path: guide for Team 7 dataset usage
    else:
        print("SPT module is ready.")
        ...
        print("from spt_final import run_spt")
        print()
        print("jobs = get_small_jobs()")
        print("results, metrics, runtime = run_spt(jobs)")
